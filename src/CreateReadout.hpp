  ...
  for (const auto& qi : queues) {
    if (qi.name == "raw_input") {
      ...
      if (inst.find("pds_list") != std::string::npos) {
        ...
        auto readout_model = std::make_unique<rol::ReadoutModel<fdt::DAPHNE_SUPERCHUNK_STRUCT,
      ...
      if (inst.find("sw_tp") != std::string::npos) {
        ...
        auto readout_model = std::make_unique<
          rol::ReadoutModel<fdt::SW_WIB_TRIGGERPRIMITIVE_STRUCT,
      ...
      if (inst.find("raw_tp") != std::string::npos) {
        ...
        auto readout_model = std::make_unique<
          rol::ReadoutModel<fdt::RAW_WIB_TRIGGERPRIMITIVE_STRUCT,
                       rol::EmptyFragmentRequestHandlerModel<fdt::RAW_WIB_TRIGGERPRIMITIVE_STRUCT,
